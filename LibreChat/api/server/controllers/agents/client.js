...
const { getDateStr, getTimeStr } = require('~/server/services/createRunBody');
...
class AgentClient extends BaseClient {
  ...
  async buildMessages(
    ...
    const { clientTimestamp } = this.options.req.body;
    const dateStr = getDateStr(clientTimestamp);
    const timeStr = getTimeStr(clientTimestamp);
    const dateTimeInstruction = `Current date and time: ${dateStr} ${timeStr}`;
    systemContent = `${dateTimeInstruction}\n\n${systemContent}`;
