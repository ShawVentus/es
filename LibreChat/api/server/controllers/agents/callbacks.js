...
const { mcpDebug } = require('~/server/utils/mcpDebugLogger');

// [FIX] Cache for recovering lost tool names (key: run_id, value: Map<index, name>)
...
class ModelEndHandler {
  ...
  async handle(event, data, metadata, graph) {
    ...
    let errorMessage;
    try {
      // [debug] ModelEndHandler Log
      if (process.env.MCP_DEBUG === 'true') {
         mcpDebug('MODEL_END', { event, output: data?.output, metadata: { run_id: metadata?.run_id } });
      }
      ...
      // [FIX] Attempt to recover invalid tool calls (e.g. Qwen missing ID or malformed args)
      if (data?.output?.invalid_tool_calls?.length > 0) {
        ...
        for (const invalid of data.output.invalid_tool_calls) {
          try {
            // If args is a string (JSON), try to parse it
            let args = invalid.args;
            if (typeof args === 'string') {
              args = JSON.parse(args);
            }

            // If we have an object now, it's valid args
            if (args && typeof args === 'object') {
              validTools.push({
                name: invalid.name || 
                        (toolNameCache.has(metadata?.run_id) && toolNameCache.get(metadata.run_id).get(0)) || // Try simple retrieval for index 0
                        '',
                args,
                id: invalid.id || `call_${nanoid()}`, // Ensure ID exists
                type: 'tool_call'
              });
              // [debug] Recovered tool call
              if (process.env.MCP_DEBUG === 'true') {
                 mcpDebug('RECOVERED_TOOL', { original: invalid, recovered: validTools[validTools.length - 1] });
              }
            } else {
              stillInvalid.push(invalid);
            }
          } catch (e) {
            stillInvalid.push(invalid);
          }
        ...
        data.output.tool_calls.push(...validTools);

        data.output.invalid_tool_calls = stillInvalid;
      }
      
      // [FIX] Cleanup cache
      if (metadata?.run_id) {
        toolNameCache.delete(metadata.run_id);
      }
...
function getDefaultHandlers({
  ...
  const handlers = {
    ...
    [GraphEvents.ON_RUN_STEP_DELTA]: {
       ...
       */
      handle: (event, data, metadata) => {
        // [debug] LLM Stream Log
        if (process.env.MCP_DEBUG === 'true') {
           mcpDebug('LLM_STREAM_DELTA', { event, type: data?.delta?.type, delta: data?.delta, metadata });
        }

        // [FIX] Cache tool name from delta if present
        if (data?.delta?.tool_calls?.length > 0 && metadata?.run_id) {
          const runId = metadata.run_id;
          data.delta.tool_calls.forEach((tc) => {
            if (tc.name && tc.index !== undefined) {
               if (!toolNameCache.has(runId)) {
                 toolNameCache.set(runId, new Map());
               }
               toolNameCache.get(runId).set(tc.index, tc.name);
            }
          });
        }
    ...
    [GraphEvents.ON_MESSAGE_DELTA]: {
       ...
       */
      handle: (event, data, metadata) => {
        // [debug] LLM Message Delta Log
        if (process.env.MCP_DEBUG === 'true') {
           mcpDebug('LLM_MSG_DELTA', { event, delta: data?.delta, metadata: { run_id: metadata?.run_id, langgraph_node: metadata?.langgraph_node } });
        }
