...
async function createMCPTool({
  ...
  mcpDebug('CREATE_TOOL_ENTER', { toolKey, toolName, serverName, userId: user?.id });
  ...
  if (serverConfig?.url) {
    ...
    const allowedDomains = appConfig?.mcpSettings?.allowedDomains;
    const isDomainAllowed = await isMCPDomainAllowed(serverConfig, allowedDomains);
    // [debug] Domain validation log
    mcpDebug('DOMAIN_CHECK', { serverName, url: serverConfig.url, isDomainAllowed });
    if (!isDomainAllowed) {
...
function createToolInstance({
  ...
  const _call = async (toolArguments, config) => {
    ...
    // [debug] _call entry point with full arguments
    mcpDebug('_CALL_ENTER', {
      toolName,
      serverName,
      userId,
      toolArguments,
      stepId,
      config: { ...config, signal: undefined }, // avoid serializing signal
    });
    ...
    try {
      ...
      // [debug] Before calling MCPManager
      mcpDebug('BEFORE_MCP_MANAGER', {
        serverName,
        toolName,
        provider,
        toolArguments,
      ...
      // [debug] After calling MCPManager
      mcpDebug('AFTER_MCP_MANAGER', {
        serverName,
        toolName,
        resultType: Array.isArray(result) ? 'Array' : typeof result,
        resultSummary: typeof result === 'string' ? result.substring(0, 100) : 'Object',
      });
