...
class YahooAdapter(BaseDataAdapter):
    ...
    def __init__(self, cache, proxy_url: str = None):
        ...
        self.cache = cache
        self.logger = logger
        self.proxy_url = proxy_url  # Trust the injected value (None means disabled)

        # Configure proxy
        # yfinance 1.0+ uses curl_cffi which requires proxy as dict format
        if self.proxy_url:
            self.logger.info(f"🔌 Configuring Yahoo Adapter proxy: {self.proxy_url}")
            
            # 1. Always set environment variables as fallback/baseline
            # This ensures requests/aiohttp and other libs respect the proxy
            import os
            ...
            # 2. Configure yfinance specific proxy
            try:
                proxy_dict = {
                    "http": self.proxy_url,
                ...
                self.logger.info(
                    f"✅ Yahoo adapter configured with proxy (yf config + env vars): {self.proxy_url}"
                )
            except Exception as e:
                self.logger.warning(f"⚠️  Failed to set proxy via yf.config: {e} (Env vars still set)")
        else:
    ...
    async def search_assets(self, query: AssetSearchQuery) -> List[AssetSearchResult]:
        """Search using Yahoo Finance API via curl_cffi to bypass WAF."""
        cache_key = f"yahoo:search:{query.query}"
        cached = await self.cache.get(cache_key)
        if cached:
        ...
        params = {
            "q": query.query,
            "quotesCount": max(query.limit, 10),
            "newsCount": 0,
            "enableFuzzyQuery": "false",
            "quotesQueryId": "tss_match_phrase_query"
        }
        
        # curl_cffi handles user-agent, just add referer
        headers = {
        ...
        for attempt in range(3):
            try:
                ...
                async with AsyncSession(impersonate="chrome110", proxies=proxies) as client:
                    resp = await client.get(url, params=params, headers=headers, timeout=10)
                    if resp.status_code == 429:
                        self.logger.warning(f"Yahoo search rate limited (429), retrying {attempt+1}/3...")
                ...
                for quote in quotes[:query.limit]:
                    # Skip if not stock or index or etf
                    ...
                    # Internal ticker conversion logic
                    if symbol.endswith(".HK"):
                        internal_ticker = f"HKEX:{symbol[:-3]}"
                    else:
                        if exch in ["NYQ", "NMS", "ASE", "NGM", "NCM"]:
                            if exch == "NYQ":
                                 internal_ticker = f"NYSE:{symbol}"
                            else:
                                 internal_ticker = f"NASDAQ:{symbol}"
                        elif "." not in symbol:
                            internal_ticker = f"NYSE:{symbol}"
                        else:
                            internal_ticker = symbol
                    ...
                    results.append(
                        AssetSearchResult(
                            ...
                            name=shortname,
                            exchange=exch,
                            country="US" if "." not in symbol else "Global",
                            currency="USD" if "." not in symbol else "Unknown",
                            relevance_score=quote.get("score", 0.5)
                        )
                    )
                ...
                if attempt == 2:
                    self.logger.warning(f"Yahoo search failed after 3 attempts: {e}")
                    return []
