...
from server.domain.services.chart_generator import ChartGenerator, CHINESE_FONT_FALLBACK
import tempfile
import numpy as np
...
def test_chinese_font():
    ...
    print("中文字体显示测试")
    print("=" * 60)
    print(f"当前字体回退链: {CHINESE_FONT_FALLBACK}")
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        print("\n[3/7] 测试偏自相关函数图...")
        try:
            pacf_data = [1, 4, 2, 5, 3, 6, 4, 2, 5, 7, 3, 8, 4, 6, 5] * 2
    ...
    if missing_glyph_warnings:
        print("\n❌ 检测到中文 glyph 缺失警告：")
        for message in missing_glyph_warnings[:10]:
            print(f"  - {message}")
        test_results.append(('中文glyph警告', '\n'.join(missing_glyph_warnings), False))
